...
2. 并发标记：不独占不STW，进行GC RootsTracing过程，可达性分析。

3. 重新标记：短暂。为了修正并发标记过程，程序运行产生的新对象。比初始标记稍长，但远比并发标记短。

4. 并发清除：不独占不STW，和用户应用线程一起存活，不挂起应用线程。所以，总的来说，CMS的内存回收线程与应用线程并发执行。

   这一阶段，由于并发，会产生`浮动垃圾`，因为即使CMS已经在并发清除，但是应用线程这个阶段产生的垃圾在标记之后，<u>不会在当次收集中处理掉，只会在下一次被回收</u>。

> `-XX:+UserConcMarkSweepGC`  表示新生代用ParNew，老年代用CMS。（比较常用）
>
> -XX:CMSInitialOccupyFraction  因为CMS是并行并发收集，所以不能等老年代几乎满了才触发，要不然这时候应用线程就会报OOM等异常了。这个参数就是设置当老年区到达指定内存一定比率的时候触发CMS。早期版本中，默认是68%。
>
> -XX:CMSInitiatingOccupancyFraction 

G1

JDK1.7首次引用，1.8正式商用。特点是跨新生代和老年代都可以回收，并发与并行的垃圾收集器。
